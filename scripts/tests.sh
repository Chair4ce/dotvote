...
function acceptanceTests {
    ...
    java -jar -Dspring.profiles.active=test ${BASE_DIR}/target/dotvote-[0-9\.]*-SNAPSHOT.jar --server.port=9090 &> ${BASE_DIR}/tmp/acceptance.log &
    echo $! > ${BASE_DIR}/tmp/dotvote.pid

    testConnection ${REACT_APP_HOST} $(cat ${BASE_DIR}/tmp/dotvote.pid)

    pushd ${BASE_DIR}/acceptance
        yarn install
#        if [[ "${DOTVOTE_CI}" && "$(lsb_release -crid | grep -i 'Ubuntu')" ]]; then
...
function cleanup {
    showBanner "Cleanup"
    if [[ -f ${BASE_DIR}/tmp/dotvote.pid ]]; then
        cat ${BASE_DIR}/tmp/dotvote.pid | xargs kill -9
        rm ${BASE_DIR}/tmp/dotvote.pid
    fi
}
trap cleanup EXIT
...
function jarBuild {
    showBanner "Build JAR"
       pushd ${BASE_DIR}
        mvn -Dmaven.test.skip=true -DskipTests -Dflyway.user=${DOTVOTE_DB_USERNAME} -Dflyway.password= -Dflyway.url=${DOTVOTE_DB_URL} clean flyway:migrate package
